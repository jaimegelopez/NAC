...
print(gcf,'-dpng','supp_figure_alternative_growth.png','-r1200');


%% Make plot with varying saturation

clear;clc

altfun = @(x,pstar) 1./((1/size(x,1)).*sum((x+pstar)./x,1));
%altfun = @(x,pstar) sum(x./(pstar + x),1);
%prot_mean = (5.2*6.4);
n1 = 50;
%folds = logspace(-3,3,n1);
%pstar = prot_mean.*folds;
pstar = 1;
mean_range = logspace(-7,3,n1);
n2 = 100000;
cv = 0.4;
k = (1/cv)^2;
n_met = 7;   
for i = 1:n1
        theta = mean_range(i)/k;
        data = gamrnd(k,theta,n_met,n2);
        g(i) = mean(altfun(data,pstar));
        max_growth(i) = altfun(mean_range(i),pstar);
        %max_growth(i) = n_met*altfun(mean_range(i),pstar);
end

growth_gap = 1 -  g./max_growth;
figure
plot(mean_range,growth_gap,'k-','LineWidth',1.5);
set(gca,'Xscale','log')
ylabel('Fraction of growth lost to noise')
xlabel('Mean metabolite level (half saturation = 1)')
title('Growth loss vs. mean metabolite level with constant CV')

print(gcf,'-dpng','growth_loss_vs_mean_metabolte.png','-r600');
